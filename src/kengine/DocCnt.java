...
public class DocCnt implements Comparable {
  ...
  /**
   * @effects If <code>x = null</code> throws <code>NullPointerException</code>, 
   *          else if <code>x</code> is not an instance of <code>DocCnt</code> throws
   *          <code>ClassCastException</code>, else if 
   *          <code>this.d.title() = x.d.title() && this.d.cnt = x.cnt</code> returns 
   *          true, else returns false
   */
  public boolean equals(Object x) {
    if (x == null) throw new NullPointerException("DocCnt.equals: argument is null");
    
    if (!(x instanceof DocCnt)) 
      throw new ClassCastException("DocCnt.equals: argument is not a DocCnt object");
    
    DocCnt dc = (DocCnt) x;
    
    return (dc.d.title().equals(this.d.title()) && 
        dc.cnt == this.cnt);
  }
}
